...
define([
    "Base", "Cell", "CellSpace", "three", "../lib/Sound", "dat-gui", "TrackballControls"
], function (
    Base, Cell, CellSpace, THREE, Sound, DAT
) {
    'use strict';
    ...
    var Petri = function  (options) {
        ...
        gui.add(params, 'y').min(-1*self.screenHeight).max(self.screenHeight).step(1).onFinishChange(function(v){
            self.cameraPosition.y = v;
            self.camera.position.set(self.cameraPosition.x, self.cameraPosition.y, self.cameraPosition.z );
        });
        gui.add(params, 'z').min(-1*self.screenDepth).max(self.screenDepth).step(1).onFinishChange(function(v){
        ...
        for (var i=0; i<this.sporesAt.length; i++){
            ...
            var cell = new Cell({
                sound       : this.getSound(
                    0, // parseInt(Math.random(1)*this.samplesSubDirs.length),
                    0  // parseInt(Math.random(1)*this.samplesScale.length)
                ),
                geometry     : geometry,
                material     : material,
    ...
    Petri.prototype.options = {
        ...
        screenDepth         : window.innerHeight*5,
        fov                 : 45,
